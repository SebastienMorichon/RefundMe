...
type AdministrativeCase = { id: string; status: string; fulfillmentMode: "SELF_SERVICE" | "MANAGED_POSTAL" | null; estimatedRecoverableCents: number; serviceFeeCents: number; confidence: number | null; rule: { id: string; name: string; organizer: string } | null };
...
function formatStatus(status: string): string { const labels: Record<string, string> = { UPLOADED: "Déposé", ANALYZED: "Analysé", DRAFT: "À préparer", WAITING_FOR_USER_DOCUMENTS: "Pièces attendues", READY_TO_PAY: "Dossier complet", GENERATED: "Prêt à envoyer", PAID: "Payé", PRINT_READY: "En préparation", SENT: "Envoyé", REFUNDED: "Remboursé", REJECTED: "Refusé" }; return labels[status] ?? status; }
function formatNextAction(status: string): string { return status === "DRAFT" ? "Préparer votre dossier" : status === "WAITING_FOR_USER_DOCUMENTS" ? "Ajouter les pièces demandées" : status === "READY_TO_PAY" ? "Choisir votre mode d’envoi" : status === "GENERATED" ? "Envoyer votre dossier" : status === "SENT" ? "Suivre la demande" : "Consulter le dossier"; }
