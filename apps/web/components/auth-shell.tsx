...
export function AuthShell({ children }: { children: ReactNode }) {
  return (
    <main className="min-h-screen bg-white lg:grid lg:grid-cols-[0.92fr_1.08fr]">
      <section className="relative hidden min-h-screen overflow-hidden border-r border-[#dce5e0] bg-[#f3f6fa] lg:block">
        <Image src="/illustrations/lydi-hero.png" alt="Lydi guide une facture vers un dossier complet" fill priority sizes="42vw" className="object-cover object-center" />
        <div className="absolute inset-x-0 bottom-0 bg-[#17211d] px-10 py-9 text-white">
      ...
      <section className="flex min-h-screen flex-col">
        <header className="flex h-[72px] items-center justify-between border-b border-[#e2e8f0] px-5 sm:px-8">
          ...
          <a href="/" className="text-sm font-bold text-[#59665f] hover:text-[#087a55]">Retour à l’accueil</a>
        </header>
