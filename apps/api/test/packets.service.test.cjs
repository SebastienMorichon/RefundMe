...
test("accepts the analyzed operator invoice as the required purchase proof", () => {
  const required = [
    {
      kind: "PURCHASE_PROOF",
      label: "facture détaillée complète de l'opérateur mobile avec les numéros SMS facturés",
      required: true,
    },
    { kind: "IDENTITY_DOCUMENT", label: "copie de la pièce d'identité du titulaire", required: true },
    { kind: "BANK_DETAILS", label: "RIB du titulaire de la ligne", required: true },
  ];

  assert.deepEqual(
    findMissingRequiredDocumentLabels(required, ["ORANGE_INVOICE", "IDENTITY_DOCUMENT", "BANK_DETAILS"]),
    [],
  );
  assert.deepEqual(
    findMissingRequiredDocumentLabels(required, ["IDENTITY_DOCUMENT", "BANK_DETAILS"]),
    ["Facture opérateur"],
  );
});
...
test("adds the optional postage and printing request to the letter", () => {
  const paragraphs = postalExpenseClaimParagraphs({
    ...
    terms: {
      ...
      postage: { reimbursable: true, amountCents: null, basis: "tarif lettre verte moins de 20 g" },
      printing: { reimbursable: true, centsPerPage: 15, maxPages: 10, basis: "" },
      claimLimit: { scope: "PER_HOUSEHOLD_PER_GAME", strict: true, details: "une seule demande par foyer et par jeu" },
...
test("generates a neutral one-page reimbursement letter preview", async () => {
  ...
  assert.equal(pdf.getTitle(), "Demande de remboursement - Offre de remboursement test");
  assert.equal(pdf.getAuthor(), "client@example.com");
  assert.equal(pdf.getSubject(), "Demande personnelle de remboursement des frais de participation");
...
test("generates a final packet with attached document pages", async () => {
  ...
  assert.equal(pdf.getTitle(), "Demande de remboursement - Frais SMS+ detectes");
  assert.equal(pdf.getAuthor(), "client@example.com");
});
